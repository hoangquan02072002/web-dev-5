...
/*let xhr = new XMLHttpRequest();
xhr.open("GET", "http://cat-facts-api.std-900.ist.mospolytech.ru/facts", true);
xhr.onload = function () {
  if (this.status === 200) {
...
function filterList() {
  ...
  const ListItem = document.querySelectorAll('.facts-list-item');
...
function autocompleteMatch(input) {
  ...
  return ListItem.filter(function(term) {
	  if (term.match(reg)) {
  	  return term;
	  }
  });
